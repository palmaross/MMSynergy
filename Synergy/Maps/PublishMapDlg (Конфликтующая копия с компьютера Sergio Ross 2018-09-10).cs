...
namespace Maps
{
    public partial class PublishMaptDlg : Form
    {
        ...
        private void BtnOK_Click(object sender, EventArgs e)
        {
            ...
            MMBase.TRACE("cp1: CheckInternetAndProcess: " + aStorage + ", " + aProcess + "\r\n");
            ...
            // Share map
            using (ShareDlg _dlg = new ShareDlg(MMUtils.ActiveDocument.Name, aPath))
            {
        ...
        void GetProjects()
        {
            using (ProjectsDB _db = new ProjectsDB())
            {
                string _place = comboMyPlaces.Text;
                DataTable _dt = _db.ExecuteQuery("select * from PROJECTS where PLACENAME=`" + _place + "` order by PROJECTNAME");
                if (_dt.Rows.Count != 0)
                {
                    foreach (DataRow _row in _dt.Rows)
