...
namespace SynManager
{
    public class Internet
    {
        ...
        {
            MMBase.TRACE("cp2: searching process: " + aStorage + ", " + aProcess + " in processes:\r\n\r\n");

            Process[] processlist = Process.GetProcesses();
            foreach (Process theprocess in processlist)
            {
                MMBase.TRACE(theprocess.ProcessName.ToString() + "\r\n");
            }

            if (Process.GetProcessesByName(aProcess).Any())
            {
                MMBase.TRACE("cp3: process found");
                return true;
            }
            else
            {
                MMBase.TRACE("cp3: process not found");
                return false;
            }
        }
